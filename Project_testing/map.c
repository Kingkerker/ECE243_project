...
void draw_obstacle(struct obstacle* array[], int pixel_value[320][240])
{
    ...
    while(num != 7)
    {
        ...
        if(num == 0)  // First obstacle - no overlap checking needed
        {
            ...
            // draw_line(array[num]->startX, array[num]->startY, array[num]->endX, array[num]->endY, 0xffff, pixel_value);
            for(int i=array[num]->startX; i<array[num]->endX; i++)
            {
                for(int j=array[num]->startY; j<array[num]->endY; j++)
                {
                    pixel_value[i][j] = 1;
        ...
        else  // Subsequent obstacles - must check for overlaps
        {
            ...
            while(fit != num)
            {
                ...
                int centreX = (rand() % 214) + 53;   // Range: 63 to 256
                int centreY = (rand() % 134) + 53;   // Range: 63 to 176
...
void set_obstacle_pos(struct obstacle* array[], int centreX, int centreY, int mode, int num)
{
    if(mode == 0) // Horizontal obstacle (line runs left-right)
    {
        ...
        array[num]->startY = centreY - 2;
        array[num]->endX = centreX + 50;
        array[num]->endY = centreY + 2;
        
        // Set bounding box (includes 5-pixel margin around the line)
        array[num]->maxX = array[num]->endX + 10;  // 50 + 50 + 5 = 105 from start
        array[num]->maxY = array[num]->endY + 10;    // 5 pixels below the line
        array[num]->minX = array[num]->startX - 10;    // 5 pixels left of start
        array[num]->minY = array[num]->startY - 10;    // 5 pixels above the line
    }
    else // Vertical obstacle (line runs up-down)
    {
        // Set line endpoints (40 pixels above and below center)
        array[num]->startX = centreX - 2;
        array[num]->startY = centreY - 40;
        array[num]->endX = centreX + 2;
        array[num]->endY = centreY + 40;
        
        // Set bounding box (includes 5-pixel margin around the line)
        array[num]->maxX = array[num]->endX + 10;    // 5 pixels right of the line
        array[num]->maxY = array[num]->endY + 10;   // 40 + 40 + 5 = 85 from start
        array[num]->minX = array[num]->startX - 10;    // 5 pixels left of the line
        array[num]->minY = array[num]->startY - 10;    // 5 pixels above the line
    }
}
